...
const USDC_ADDRESSES: Record<number, Address> = {
  ...
  84532: BASE_SEPOLIA_USDC as Address,
};
...
export async function approveUsdc(
  ...
  // Source the live asset from the vault itself. The deployed Base Sepolia vault
  // currently points at Mock USDC, and hardcoded token constants can drift.
  const assetAddress = await getVaultAssetAddress(vaultId, chainId);
...
export async function readUserRequest(
  vaultId: string,
  controller: Address,
  chainId: number,
) {
