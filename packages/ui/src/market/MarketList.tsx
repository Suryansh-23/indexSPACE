...
export function MarketList({ markets, onSelect, loading, error, emptyMessage }: MarketListProps) {
  const ctx = useContext(FunctionSpaceContext);
  if (!ctx) throw new Error('MarketList must be used within FunctionSpaceProvider');

  if (loading) {
    return (
      <div className="fs-market-list-loading">
  ...
  if (error) {
    return (
      <div className="fs-market-list-error">
        {error.message}
      </div>
    );
  }
  ...
  if (markets.length === 0) {
    return (
      <div className="fs-market-list-empty">
