...
title: "TradePanel"
sidebar_position: 1
description: "TradePanel with Gaussian and Range shape modes using slider-based inputs for belief construction."
---
...
<WidgetDemo title="Live Demo" height="auto" maxWidth="380px">
...
* **Prediction passed to `useBuy`'s `execute()`:** In Gaussian mode, the slider value. In Range mode, the midpoint of the selected range.
...
**Internal calls:** `useMarket`, `useBuy`, `usePreviewPayout`, `generateGaussian`, `generateRange`
