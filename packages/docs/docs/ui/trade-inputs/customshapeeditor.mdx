...
sidebar_position: 4
description: "CustomShapeEditor for freeform belief drawing by dragging control points on a density chart."
---
...
* **Prediction:** The mode (peak) of the belief distribution is derived from `useCustomShape` (which internally uses `computeStatistics`) and passed to `useBuy`'s `execute()`.
...
**Internal calls:** `useMarket`, `useConsensus`, `useCustomShape`, `useBuy`, `usePreviewPayout`, `evaluateDensityCurve`, `useChartZoom`, `rechartsPlotArea`
