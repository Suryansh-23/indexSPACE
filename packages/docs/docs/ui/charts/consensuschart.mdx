...
sidebar_position: 2
description: "ConsensusChart rendering probability density with trade preview and position overlays from context."
---
...
<WidgetDemo title="Live Demo" height="350px">
  <ConsensusChart marketId={DEMO_MARKET_ID} height={300} />
</WidgetDemo>
...
1. **Consensus area** -- `type="linear"`, solid stroke, gradient fill (40% to 5% opacity). No animation.
...
3. **Selected position area** -- `type="linear"`, solid stroke, gradient fill, 300ms animation. Appears when a `PositionTable` row is clicked.
...
5. **Overlay curves** -- `type="linear"`, custom colors, gradient fills, 300ms animation.
