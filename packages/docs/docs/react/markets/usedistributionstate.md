...
* This hook is designed for sharing state between components. Pass the returned object to both a chart and a selector to keep bucket configuration synchronized. Buckets and percentiles recompute reactively when the underlying market or consensus data changes or when `bucketCount` is updated.
