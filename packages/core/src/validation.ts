 ...
 *
 * Checks (in order):
 * 1. Length matches K+1 (market's num_buckets + 1)
 * 2. All elements are finite (no NaN or Infinity)
 * 3. All elements non-negative
 * 4. Elements sum to 1.0 within tolerance (1e-6)
 */
export function validateBeliefVector(vector: number[], K: number): void {
  if (vector.length !== K + 1) {
    throw new Error(
      `Belief vector length ${vector.length} does not match expected K+1 = ${K + 1}`,
    );
  }
